...
class DataAugment(object):
    def __init__(self, img_dir, image_id=""):
        self.add_saltNoise = True
        self.gaussianBlur = True
        self.changeExposure = True
        self.id = image_id.split(".")[0]
        self.out_dir = os.path.join(img_dir, "DataAugment")
        if not os.path.exists(self.out_dir):
            os.mkdir(self.out_dir)
        img = cv.imread(os.path.join(img_dir, str(self.id)+'.png'))
        self.src = img
        ...
        self.flip_y = dst2
        self.flip_x_y = dst3
        cv.imwrite(os.path.join(self.out_dir, str(self.id)+'_flip_x'+'.png'), self.flip_x)
        cv.imwrite(os.path.join(self.out_dir, str(self.id)+'_flip_y'+'.png'), self.flip_y)
        cv.imwrite(os.path.join(self.out_dir, str(self.id)+'_flip_x_y'+'.png'), self.flip_x_y)
    ...
    def gaussian_blur_fun(self):
        if self.gaussianBlur:
            ...
            cv.imwrite(os.path.join(self.out_dir, str(self.id)+'_Gaussian'+'.png'), dst1)
            cv.imwrite(os.path.join(self.out_dir, str(self.id)+'_flip_x'+'_Gaussian'+'.png'), dst2)
            cv.imwrite(os.path.join(self.out_dir, str(self.id)+'_flip_y'+'_Gaussian'+'.png'), dst3)
            cv.imwrite(os.path.join(self.out_dir, str(self.id)+'_flip_x_y'+'_Gaussian'+'.png'), dst4)
    ...
    def change_exposure_fun(self):
        if self.changeExposure:
            ...
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_ReduceEp'+'.png'), dst1)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_x'+'_ReduceEp'+'.png'), dst3)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_y'+'_ReduceEp'+'.png'), dst5)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_x_y'+'_ReduceEp'+'.png'), dst7)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_IncreaseEp'+'.png'), dst2)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_x'+'_IncreaseEp'+'.png'), dst4)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_y'+'_IncreaseEp'+'.png'), dst6)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_x_y'+'_IncreaseEp'+'.png'), dst8)
    ...
    def add_salt_noise(self):
        if self.add_saltNoise:
            ...
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_Salt'+'.png'), dst1)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_x'+'_Salt'+'.png'), dst2)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_y'+'_Salt'+'.png'), dst3)
            cv.imwrite(os.path.join(self.out_dir,str(self.id)+'_flip_x_y'+'_Salt'+'.png'), dst4)
    ...
    def json_generation(self):
        ...
        for image_name in image_names:
            # with open(os.path.join(img_dir, image_name+".png"), "rb")as b64:
            ...
            with open(os.path.join(img_dir,str(self.id)+".json"), 'r')as js:
                ...
                json.dump(json_data, open(os.path.join(self.out_dir, image_name+".json"), 'w'), indent=2)
...
if __name__ == "__main__":
    ...
    for img in os.listdir(img_dir):
        ...
        if img.endswith(".bmp") or img.endswith(".png"):
            continue
        dataAugmentObject = DataAugment(img_dir, img)
